...
const listYay = document.querySelector(".poke-items");
// 3. The form where we intake pokemon
const formNice = document.querySelector(".add-pokemon");
// 4. The form text element that has the name the user provided
const nameCool = document.querySelector("#pokemon-name");
...
function addPokemonItem(event) {
  ...
  const pokemonName = nameCool.value;
...
function renderList() {
  ...
  while (listYay.firstChild) {
    listYay.removeChild(listYay.firstChild);
  }
...
formNice.addEventListener("submit", addPokemonItem);
